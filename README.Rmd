...
Given an initial PLVM, we can implement CAVI using the `cavi_plvm` function. In the interest of brevity, the CAVI algorithm is run for 100 iterations only. For empirical data problems we would iterate to convergence of the ELBO, defined by `tol`.
...
First, we verify that the CAVI algorithm is maximising the ELBO. The figure omits the ELBO at the first 10 iterations of the CAVI for clarity.
